...
function(arcbuild_get_package_name var_name sdk_name version build_type)
  string(TIMESTAMP current_date "%m%d%Y")
  set(package_name_parts ${name} ${version} ${build_type} ${current_date})
...
function(arcbuild_has_mpbase_dependency var_name name)
  ...
  foreach(depend ${all_depends})
    if(depend MATCHES "(mpbase|mpbase.so|mpbase.a|mpbase.lib)$")
      set(has_depend 1)
      break()
    endif()
  endforeach()
...
function(arcbuild_collect_header_files var_name)
  set(result_paths)
  foreach(path ${ARGN})
    if(path MATCHES "\\*\\*")
      file(GLOB_RECURSE path ${path})
    elseif(path MATCHES "\\*")
      file(GLOB path ${path})
    else()
      get_filename_component(path "${path}" ABSOLUTE)
    endif()
    list(APPEND result_paths ${path})
  endforeach()
  set(${var_name} ${result_paths} PARENT_SCOPE)
endfunction()


function(arcbuild_install_header_files dst_dir)
  foreach(path ${ARGN})
    if(IS_DIRECTORY "${path}")
      install(DIRECTORY ${path} DESTINATION "${dst_dir}")
    else()
      install(FILES ${path} DESTINATION "${dst_dir}")
    endif()
  endforeach()
endfunction()
...
function(arcbuild_define_arcsoft_sdk sdk_name)
  ...
  arcbuild_collect_header_files(A_INCS ${A_INCS})
  ...
  arcbuild_install_header_files("${prefix}inc" ${A_INCS})
